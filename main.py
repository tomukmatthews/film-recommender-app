...
option = film_display_option.radio(
    label="Show me", options=["Top Films", "Personal Recommendations"], index=film_selector
)
...
if option == "Top Films":
    filtered_films = apply_filters(data=data, cast=cast, director=director, genres=genres, sort_column="imdb_score")
    ...
    if len(liked_films) == 0:
        ...
        sim_weights = [cast_weight, director_weight, keywords_weight, overview_weight, user_embedding_weight]
