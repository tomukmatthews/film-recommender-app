...
collaborative_similarity_df = pd.concat([user_embedding_similarity, missing_indices_df_index], axis=0)
collaborative_similarity_df = pd.concat([collaborative_similarity_df, missing_indices_df_cols], axis=1)
collaborative_similarity_df = collaborative_similarity_df.fillna(0)
collaborative_similarity_df.to_csv(config.BASE_SIMILARITY_PATH + "collaborative_similarity.csv", index=False)
